...
### 2. コンテキストの保持
- 直近3件のユーザー指示を常に記録
- 毎回のプロンプト実行時に過去のコンテキストを自動的に読み込み
...
4. **並列実行**: 可能な限り複数のTask toolを同時に呼び出してサブエージェントを並列実行
...
##### 新規開発・改修の場合（workflow_instructions.pyで定義）
...
##### その他のタスクの場合
開発フローは適用されず、タスクに応じて実装されている8つのサブエージェントから適切なものを選択的に使用します。メインオーケストレーターは直接コードを触らず、全ての作業をサブエージェントに委譲します。
...
- **並列実行の推奨**: Task toolを使用して複数のサブエージェントを同時に呼び出すことで並列実行を実現
- **複数エージェントの同時起動**: 同じ種類のサブエージェントでも複数のTask toolを同時に呼び出して並列実行可能
- **独立タスクの同時実行**: 独立したタスクは複数のTask toolを同時に使用してサブエージェントに並列で依頼
...
2. **調査・分析の場合**
   ...
   現在のコードベースの認証実装を調査して
   ```
   → 開発フローは適用されず、必要なサブエージェントのみを選択的に使用します
