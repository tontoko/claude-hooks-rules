...
#### 2. その他のタスクの場合
**開発フローは適用されません**。以下のようなタスクでは、目的に応じて適切なサブエージェントを選択して使用します：

- **実装調査・コード解析**: `/code-analyzer` や `/code-search-agent` を使用
- **マージコンフリクト解消**: `/merge-conflict-resolver` を使用
- **バグ修正**: 問題の調査から始め、必要に応じて `/bug-fix-agent` を使用
- **リファクタリング**: `/refactoring-agent` を使用
